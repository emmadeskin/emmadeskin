
# Hi, I'm Emma Deskin 👋

Biomedical Engineering graduate from the University of Arizona (Cum Laude, May 2026).
I build things at the intersection of hardware, software, and clinical application —
from automated ultrasound imaging systems to embedded wearables to CFD simulations.

## 🔧 What I work with
**Programming:** MATLAB · Python · C · R · Arduino IDE
**Hardware:** Teensy · Raspberry Pi · Arduino · Stepper motors · Sensors · PCB assembly
**Software:** SolidWorks · ANSYS Fluent · Verasonics Vantage · ImageJ · Meshmixer · EAGLE CAD
**Skills:** Signal processing · V&V testing · 3D printing · Laser cutting · Technical documentation

## 📁 Projects
| Project | Description | Tools |
|---|---|---|
| [Ultrasound Foot Scanner](https://github.com/emmadeskin/ultrasound-foot-scanner) | Automated 3D weight-bearing ultrasound scanner for PCFD diagnosis | MATLAB, Teensy, Verasonics, SolidWorks |
| [Carotid Hemodynamics](https://github.com/emmadeskin/carotid-hemodynamic-analysis) | CFD + physical flow visualization of carotid artery blood flow | ANSYS Fluent, ImageJ, Meshmixer |
| [Heat Stress Wearable](https://github.com/emmadeskin/heat-stress-wearable) | Wireless wearable for real-time physiological heat stress monitoring | Raspberry Pi, Arduino, SolidWorks |
| [Jumping Kinematics](https://github.com/emmadeskin/jumping-kinematics-analysis) | Motion capture analysis of jump mechanics | Kinovea, MATLAB |

## 📫 Contact
📧 emmadeskin@gmail.com
🔗 linkedin.com/in/emma-deskin
📍 Leawood, KS
<!--
**emmadeskin/emmadeskin** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
